...
ramips_eth_probe(struct net_device *dev)
{
	...
	of_get_mtd_mac_address(re->of_node, dev->dev_addr);
	ether_setup(dev);
	dev->mtu = 1500;
...
struct ramips_eth_platform_data rt3050_data = {
...
{
	const struct ramips_eth_platform_data *data = plat->dev.platform_data;
	...
	if (!data) {
		...
		if (match)
			data = (const struct ramips_eth_platform_data *) match->data;
	}
